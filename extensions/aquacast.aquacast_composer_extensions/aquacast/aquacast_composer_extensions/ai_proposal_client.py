...
class AIProposalClient:
    ...
    def propose(self, *, tank_id: str | None = None, auto_alert: dict | None = None, auto_execute: bool = False) -> dict:
    ...
    def delete_completed(self, proposal_id: str) -> dict:
        return self._delete_json(f"/api/ai/actions/{proposal_id}")

    def clear_completed(self) -> dict:
        return self._delete_json("/api/ai/actions/recent/completed")
    ...
    def _delete_json(self, path: str, query: dict | None = None) -> dict:
        suffix = f"?{urlencode(query)}" if query else ""
        request = urllib.request.Request(
            url=f"{self.base_url}{path}{suffix}",
            headers={"Accept": "application/json"},
